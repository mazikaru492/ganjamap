...
writeFileSync(
  LOG_FILE,
  `=== KUSHMAP Import Log - ${new Date().toISOString()} ===\n`,
);
...
const CITIES = [
  ...
  { en: "Bangkok Sukhumvit", th: "สุขุมวิท", lat: 13.744, lng: 100.564 },
  { en: "Bangkok Silom", th: "สีลม", lat: 13.7274, lng: 100.5347 },
  { en: "Bangkok Ekkamai", th: "เอกมัย", lat: 13.718, lng: 100.586 },
  ...
  { en: "Bangkok Ladprao", th: "ลาดพร้าว", lat: 13.814, lng: 100.562 },
  { en: "Bangkok Thonburi", th: "ธนบุรี", lat: 13.727, lng: 100.476 },
  { en: "Bangkok Bang Na", th: "บางนา", lat: 13.6659, lng: 100.6085 },
  { en: "Bangkok Ratchada", th: "รัชดา", lat: 13.77, lng: 100.568 },
  { en: "Bangkok Phrom Phong", th: "พร้อมพงษ์", lat: 13.7296, lng: 100.569 },
  { en: "Bangkok Nana", th: "นานา", lat: 13.7404, lng: 100.5536 },
  // Chiang Mai areas
  { en: "Chiang Mai", th: "เชียงใหม่", lat: 18.7883, lng: 98.9853 },
  { en: "Chiang Mai Nimman", th: "นิมมาน", lat: 18.8012, lng: 98.968 },
  ...
  { en: "Pattaya Jomtien", th: "จอมเทียน", lat: 12.871, lng: 100.879 },
  { en: "Pattaya Central", th: "พัทยากลาง", lat: 12.932, lng: 100.878 },
  { en: "Pattaya North", th: "พัทยาเหนือ", lat: 12.96, lng: 100.876 },
  // Islands and other areas
  { en: "Koh Samui", th: "เกาะสมุย", lat: 9.512, lng: 100.0136 },
  { en: "Koh Samui Chaweng", th: "หาดเฉวง", lat: 9.537, lng: 100.062 },
  { en: "Koh Samui Lamai", th: "หาดละไม", lat: 9.476, lng: 100.061 },
  ...
  { en: "Koh Lanta", th: "เกาะลันตา", lat: 7.633, lng: 99.049 },
  ...
  { en: "Pai", th: "ปาย", lat: 19.357, lng: 98.4416 },
  ...
  { en: "Udon Thani", th: "อุดรธานี", lat: 17.414, lng: 102.787 },
  { en: "Khon Kaen", th: "ขอนแก่น", lat: 16.4419, lng: 102.836 },
  { en: "Pattani", th: "ปัตตานี", lat: 6.8653, lng: 101.25 },
    ...
    const res = await fetch(
      "https://places.googleapis.com/v1/places:searchText",
      {
        ...
        headers: {
          ...
          "X-Goog-FieldMask":
            "places.id,places.displayName,places.formattedAddress,places.location,places.nationalPhoneNumber,places.websiteUri,nextPageToken",
          Referer: "https://ganjamap.vercel.app/",
        },
...
async function upsertShop(place, city) {
  ...
  const shop = {
    ...
    city: city.en
      ...
      .replace(/Phuket .*/, "Phuket")
      .replace(/Pattaya .*/, "Pattaya")
      .replace(/Chiang Mai .*/, "Chiang Mai")
      .replace(/Koh Samui .*/, "Koh Samui")
      .replace(/Krabi .*/, "Krabi"),
    lat,
    lng,
  ...
  if (existing2) {
    ...
    if (place.place_id) {
      await supabase
        .from("shops")
        .update({ google_place_id: place.place_id })
        .eq("id", existing2.id);
    }
    return false;
  }
...
async function main() {
  ...
  log(
    `Cities: ${CITIES.length}, Queries per city: ${QUERIES(CITIES[0]).length}\n`,
  );
  ...
  for (const city of CITIES) {
    ...
    for (let qi = 0; qi < queries.length; qi++) {
      ...
      try {
        ...
        log(
          `  ${city.en} ${qi + 1}/${queries.length} "${query}": ${places.length} found (${newPlaces.length} new)`,
        );
